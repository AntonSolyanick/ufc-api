...
const namesParserUFC: Function = async () => {
...
export const eventParserUfc: Function = async (fighterSlugName: string) => {
    ...
    const parsedData = await page.evaluate(() => {
        ...
        if (fightersSmallImgs) {
            ...
            secondFighterSmallImg = fightersSmallImgs[1].getAttribute('src')!
        }
...
const populateCollection: Function = async (
    ...
    for (let i = 0; i < fighters.length; i++) {
        const [fighterImage, fighterRusName, nextFightInfo] = await eventParser(
            `${fighters[i].slug}`
        )
